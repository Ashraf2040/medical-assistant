...
const List = () => {
  ...
  const [coordinates, setCoordinates] = useState({ latitude: "21.4241", longitude: "39.8173" });
  ...
  const getUserAddress = async () => {
    const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${coordinates.latitude},${coordinates.longitude}&key=AIzaSyBMCmgqs8_22JHMb_twcCXhyVBqhuthppc`);
  ...
  const getHospitalInfo = async () => {
    ...
    const response = await fetch(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=21.4241,39.8173&radius=10000&type=hospital&keyword=hospital&name=hospital&key=AIzaSyBMCmgqs8_22JHMb_twcCXhyVBqhuthppc`);
    const data = await response.json();
    console.log(data);
